<div x-data="{
    ...
    <div x-ref="panel" x-show="open" x-transition.origin.top.left x-on:click.outside="close($refs.button)"
        ...
        @foreach ($categories as $category)
            <div>
                <a href="/?category={{$category->slug}}&{{http_build_query(request()->except('category'))}}" class="block w-full px-4 py-2 text-left text-sm hover:bg-blue-700 hover:text-white">
                    {{$category->name}}
                </a>
            </div>
        @endforeach
